...
CreateImage() の代りに C<Cv::Image-E<gt>new> を、
CreateMat() の代りに C<Cv::Mat-E<gt>new> を使うことができます。
呼び出し形式について、
CreateImage() と CreateMat() は違いますが、
C<Cv::something-E<gt>new> を介した場合、違いはありません。
これは、C<Cv::Arr> の中で、元のオブジェクトのことを知らないで
同じオブジェクトを作れるようにしています。
